...
db_path=current_directory+r"/instance/tunefinder.db"
# Initialize app
#app = Flask(__name__,static_folder="web/frontend/build",static_url_path="")
app=Flask(__name__)
CORS(app, origins=['*'])
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + db_path #'sqlite:///tunefinder.db'
...
def get_all_songs():
    ...
    songs = db.session.query(Song.id, Song.title, Song.artist, Song.album, Song.youtube_link).all()
    ...
    if files:  # Check if any files are present in the folder
        ...
        if filename.endswith(".mp3") or filename.endswith(".wav"):
            ...
            songs = db.session.query(
                Song.id, Song.title, Song.artist, Song.album, Song.youtube_link).filter(Song.id.in_(song_ids)).group_by(Song.id).all()
...
def get_recommendations():
    ...
    if user:
        ...
        user_id= None
    knn_song_ids = knn_recc_data.loc[knn_recc_data['User']==user_id-1, "Recommended Songs"].values[0]
    
    # print(mf_song_ids)
    knn_song_ids = ast.literal_eval(knn_song_ids)
    song_ids = knn_song_ids 
...
def load_songs():
    songs_df = pd.read_csv('web/backend/preprocessing/SONGS_DB.csv', encoding='utf-8')
